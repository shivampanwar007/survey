  ...
  return (
    <div className="xl:px-[70px] lg:px-[50px] md:px-[30px] px-[24px] font-serif space-y-8">
      ...
      <div className="flex justify-evenly flex-wrap gap-10 ">
        {options.map((option, index) => (
          <p
            ...
            onClick={() => toggleSection(option)} // Toggle collapse on click
          >
            {collapsedSections[option] ? `+ ${option}` : `- ${option}`}
          </p>
        ))}
      </div>
      ...
      {options.map((option, index) => (
        <div
          key={index}
          ref={(el) => (sectionRefs.current[option] = el)}
          className={`bg-white p-4 rounded-2xl ${collapsedSections[option] ? "hidden" : ""}`}
